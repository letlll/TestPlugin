...
export class TableToolbar {
    ...
    private generateTableId(): void {
        try {
            ...
            if (isEditMode && this.plugin.settings.enableEditModeOperations) {
                // 在编辑模式下，提示用户需要手动添加HTML注释
                new Notice('请手动在表格前添加HTML注释: <!-- table-id: 你的表格ID -->', 10000);
                ...
                this.plugin.readTableIdFromMarkdown(this.activeTable).then(existingId => {
                    ...
                    // 不再自动添加ID，而是提示用户需要手动添加HTML注释
                    new Notice('请在编辑模式下，手动在表格前添加HTML注释: <!-- table-id: 你的表格ID -->', 10000);
                    
                    // 生成一个建议的ID供用户参考
                    const suggestedId = this.generateUniqueTableId();
                    new Notice(`建议的表格ID: ${suggestedId}`, 10000);
    ...
    setActiveEditModeTable(tableInfo: { startLine: number, endLine: number, content: string }): void {
        try {
            ...
            // 显示提示
            new Notice('编辑模式下已选中表格，可以使用工具栏进行操作', 2000);
